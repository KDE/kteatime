...
TeaListModel::TeaListModel(const QList<Tea> &tealist, QObject *parent) : QAbstractTableModel(parent), m_tealist(tealist)
{
}
...
QVariant TeaListModel::data(const QModelIndex &index, int role) const
{
    ...
    if(role==Qt::DisplayRole) {
        if(index.column()==0)
            return QVariant(m_tealist.at(index.row()).name());
        return QVariant(m_tealist.at(index.row()).timeToString());
    }

    if(role==Qt::EditRole) {
        if(index.column()==0)
            return QVariant(m_tealist.at(index.row()).name());
        return QVariant(m_tealist.at(index.row()).time());
    }
